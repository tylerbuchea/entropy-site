 ...
 */
{
  ...
  class Slideshow {
    ...
    initEvents() {
      Array.from(
        document.querySelectorAll('.grid__item--nav-next')
      ).forEach(ctrl =>
        ctrl.addEventListener('click', ev => this.navigate(ev, 'next'))
      );
      Array.from(
        document.querySelectorAll('.grid__item--nav-prev')
      ).forEach(ctrl =>
        ctrl.addEventListener('click', ev => this.navigate(ev, 'prev'))
      );
    ...
    navigate(ev, direction) {
      ...
      this.current = direction === 'next'
        ? this.current < this.gridsTotal - 1 ? this.current + 1 : 0
        : this.current > 0 ? this.current - 1 : this.gridsTotal - 1;
